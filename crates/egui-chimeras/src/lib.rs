//! egui / eframe integration for the [`chimeras`] camera library.
//!
//! This crate owns only the thin glue between a running
//! [`chimeras::pump::Pump`] and an [`egui::TextureHandle`]. Every camera
//! primitive (pause / resume, single-frame capture, hotplug, source
//! abstraction) lives upstream in [`chimeras`] itself and is re-exported here
//! for convenience.
//!
...
//! ```
//!
//! The doctest is `ignore`d because it uses `eframe`, which `egui-chimeras`
...
use chimeras::{Camera, Frame, pump};
use egui::{ColorImage, Context, TextureHandle, TextureOptions, Ui};

pub use chimeras::pump::{Pump, capture_frame, set_active, stop_and_join};

const DEFAULT_TEXTURE_NAME: &str = "chimeras-frame";
...
/// Convert a chimeras [`Frame`] into an egui [`ColorImage`] (RGBA8).
pub fn frame_to_color_image(frame: &Frame) -> Result<ColorImage, chimeras::Error> {
...
pub fn update_texture(stream: &mut Stream, ctx: &Context) -> Result<bool, chimeras::Error> {
